"""GUI for auditing scenario 33 blockage labels against camera frames (nighttime).

View frames in chronological order (by index), mark blockages as visible/not visible.
Run: .venv/Scripts/python.exe -m scripts.audit_gui_s33
Saves to outputs/scenario33_audit.json
"""
...
class AuditGUI_S33:
    def __init__(self, root):
        self.root = root
        self.root.title("Scenario 33 Blockage Audit (Nighttime)")
        self.root.geometry("1300x750")

        # Load scenario 33 frames in chronological order
        df = pd.read_csv(ROOT / "scenario33" / "scenario33_dev_labelled.csv")
        self.frames = df.sort_values("index").reset_index(drop=True)

        self.audit_path = ROOT / "outputs" / "scenario33_audit.json"
        self.load_audit()
        self.current_idx = 0
    ...
    def show_frame(self):
        ...
        path = ROOT / "scenario33" / row["unit1_rgb"]
        ...
        info = (
            ...
            f"Index {int(row['index'])} | Seq {int(row['seq_index'])} | "
            f"Derived Label: {row['label_derived'].upper()} | Status: {audit_status}"
        )
    ...
    def jump(self):
        ...
        try:
            ...
            self.current_idx = max(0, min(target, len(self.frames) - 1))
            self.show_frame()
        except ValueError:
            pass
